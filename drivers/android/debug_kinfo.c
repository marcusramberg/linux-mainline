 ...
 *
 * Ported from the Android common kernel's drivers/android/debug_kinfo.c
 * (CONFIG_ANDROID_DEBUG_KINFO). One deliberate difference: the upstream
 * version reads rmem->priv for the reserved region, which only the CMA
 * handler ever sets, so it can never bind to a "no-map" region on this SoC.
 * Map the region here instead. The published structure is unchanged.
 *
 * Why this exists: at a reset the Zumapro debug-snapshot firmware writes a
 * dump into the klog partition, and it prints
 *
 *   [LNXDBG] magic_number must be 0xcceeddff but 0x00000000
 *   [DSS] linux debug info is not valid
 *
 * and then resolves nothing -- no symbol, no backtrace, per-CPU state "N/A".
 * Filling this table is what lets it unwind the kernel from physical memory,
 * which is the only post-mortem that survives a bus wedge: consoles die with
 * the freeze, and ramoops is erased when the APM watchdog retrains DRAM.
 */
...
extern const unsigned int kallsyms_num_syms __weak __section(".rodata");
...
static int build_info_set(const char *str, const struct kernel_param *kp)
{
	...
	if (!all_info_addr || !all_info_size)
		return -EPERM;
...
static int debug_kinfo_probe(struct platform_device *pdev)
{
	...
	if (!mem_region)
		return dev_err_probe(&pdev->dev, -ENODEV, "no memory-region\n");

	rmem = of_reserved_mem_lookup(mem_region);
	of_node_put(mem_region);
	if (!rmem)
		return dev_err_probe(&pdev->dev, -ENODEV,
				     "no such reserved memory node\n");

	if (!rmem->base || rmem->size < sizeof(struct kernel_all_info))
		return dev_err_probe(&pdev->dev, -EINVAL,
				     "unexpected reserved memory (%pa@%pad)\n",
				     &rmem->size, &rmem->base);

	all_info_addr = memremap(rmem->base, rmem->size, MEMREMAP_WB);
	if (!all_info_addr)
		return dev_err_probe(&pdev->dev, -ENOMEM,
				     "failed to remap %pa@%pad\n",
				     &rmem->size, &rmem->base);
	all_info_size = rmem->size;

	memset(all_info_addr, 0, sizeof(struct kernel_all_info));
	all_info = all_info_addr;
	info = &all_info->info;
	...
	strscpy(info->last_uts_release, init_utsname()->release,
		sizeof(info->last_uts_release));
	info->enabled_modules_tree_lookup = IS_ENABLED(CONFIG_MODULES_TREE_LOOKUP);
	/*
	 * Drift since the 6.1 layout this table describes: struct module's
	 * core_layout/init_layout pair became one mem[] array indexed by
	 * mod_mem_type, so both offsets now point at the same array and a
	 * consumer written against 6.1 cannot resolve the init sections. Kernel
	 * (non-module) symbolication, which is what a suspend wedge needs, uses
	 * the kallsyms fields above and is unaffected.
	 */
	info->mod_core_layout_offset = offsetof(struct module, mem);
	info->mod_init_layout_offset = offsetof(struct module, mem);
	info->mod_kallsyms_offset = offsetof(struct module, kallsyms);
	info->module_start_va = MODULES_VADDR;
	info->module_end_va = MODULES_END;
	update_kernel_all_info(all_info);

	dev_info(&pdev->dev, "kernel info at %pa (%u syms)\n", &rmem->base,
		 info->num_syms);
...
static struct platform_driver debug_kinfo_driver = {
	...
	/* no remove: the region is reserved for the firmware's whole life */
	.driver = {
		.name = "debug-kinfo",
